@extends('admin.layouts.app',['title'=>'Gift-Page'])
@section('content')
                    ...
                    <div class="datatable-wrapper datatable-loading no-footer sortable searchable fixed-columns">
                                    ...
                                    <tr>
                                        <td class="lg:w-1/6 md:w-1/6 sm:w-1/6 xs:w-1/6">
                                            <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
                                                <p class="text-sm font-medium text-black dark:text-white">
                                                    @if($gift->image)
                                                    <img src="{{ asset('storage/images/' . $gift->image) }}"
                                        ...
                                        <td class="lg:w-1/6 md:w-1/6 sm:w-1/6 xs:w-1/6">
                                            <label class="relative inline-flex items-center cursor-pointer">
                                                <input type="checkbox" data-id="{{ $gift->id }}" name="status"
                                                    class="js-switch" {{ $gift->status == 1 ? 'checked' : '' }} value=""
                        ...
                        <div class="datatable-bottom">
                            <div class="datatable-info">
                                Showing {{ $gifts->firstItem()}} to
                                {{ $gifts->lastItem() }} of
                                {{ $gifts->total() }} entries
                            </div>
                            {{ $gifts->appends($_GET)->links('vendor.pagination.custom') }}
                        </div>
                    </div>
                </div>
            </div>
