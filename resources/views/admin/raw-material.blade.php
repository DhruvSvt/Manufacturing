@extends('admin.layouts.app',['title'=>'Raw Material-Page'])
@section('content')
                    ...
                    <div class="datatable-wrapper datatable-loading no-footer sortable searchable fixed-columns">
                            ...
                            <table class="table w-full table-auto datatable-table" id="dataTableTwo">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="lg:w-1/6 md:w-1/6 sm:w-1/6 xs:w-1/6">
                                            <p class="text-sm font-medium text-black dark:text-white">
                                                {{ $rm->parent ? $rm->parent->short_name : '-' }}
                                            </p>
                                        </td>
                                        <td class="lg:w-1/6 md:w-1/6 sm:w-1/6 xs:w-1/6">
                                            <p class="text-sm font-medium text-black dark:text-white">
                                        ...
                                        <td class="lg:w-1/6 md:w-1/6 sm:w-1/6 xs:w-1/6">
                                            <label class="relative inline-flex items-center cursor-pointer">
                                                <input type="checkbox" data-id="{{ $rm->id }}" name="status"
                                                    class="js-switch" {{ $rm->status == 1 ? 'checked' : '' }} value=""
                                        ...
                                        <td class="lg:w-1/6 md:w-1/6 sm:w-1/6 xs:w-1/6">
                                            <div class="flex items-center space-x-3.5">
                                                <a href="{{ route('raw-material.edit',$rm->id) }}">
                        ...
                        <div class="datatable-bottom">
                            <div class="datatable-info">
                                Showing {{ $raw_material->firstItem()}} to
                                {{ $raw_material->lastItem() }} of
                                {{ $raw_material->total() }} entries
                            </div>
                            {{ $raw_material->appends($_GET)->links('vendor.pagination.custom') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    let elems = Array.prototype.slice.call(document.querySelectorAll('.js-switch'));
        ...
        $(document).ready(function() {
            $('.js-switch').change(function() {
                ...
                let raw_materialId = $(this).data('id');
                $.ajax({
                    type: "POST", // Change this to POST
                    dataType: "json",
                    url: "{{ route('raw-material.status')}}",
                    data: {
